...
class ProductModel {
  ...
  factory ProductModel.fromJson(Map<String, dynamic> json) {
    ...
    if (setting.useViewGalleries == true) {
      galleries =
          List.from(json['galleries'] ?? []).map((item) {
    ...
    if (setting.useViewFeature == true) {
      features =
          List.from(json['features'] ?? []).map((item) {
            return CategoryModel.fromJson(item);
          }).toList();
    }

    final listRelated =
        List.from(json['related'] ?? []).map((item) {
          return ProductModel.fromJson(item);
        }).toList();

    final listLatest =
        List.from(json['lastest'] ?? []).map((item) {
          return ProductModel.fromJson(item);
        }).toList();

    if (setting.useViewOpenHours == true) {
      openHours =
          List.from(json['opening_hour'] ?? []).map((item) {
    ...
    if (setting.useViewAttachment == true) {
      attachments =
          List.from(json['attachments'] ?? []).map((item) {
