...
final Map<String, ProductViewType> mapListView = {
...
class SettingModel {
  final ListingStyleFactory businessFactory;
  final bool useLayoutWidget;
  ...
  SettingModel({
    required this.businessFactory,
    required this.useLayoutWidget,
  ...
  factory SettingModel.fromDefault() {
    return SettingModel(
      businessFactory: ListingCreator('basic').create(),
  ...
  factory SettingModel.fromJson(Map<String, dynamic> json) {
    ...
    final layout = settings['layout_mode'] ?? 'basic';

    return SettingModel(
      businessFactory: ListingCreator(layout).create(),
      useLayoutWidget: settings['layout_widget'] ?? false,
      category:
          List.from(json['categories'] ?? []).map((e) {
            return CategoryModel.fromJson(e);
          }).toList(),
      features:
          List.from(json['features'] ?? []).map((e) {
            return CategoryModel.fromJson(e);
          }).toList(),
      locations:
          List.from(json['locations'] ?? []).map((e) {
            return CategoryModel.fromJson(e);
          }).toList(),
      sort:
          List.from(json['place_sort_option'] ?? []).map((item) {
            return SortModel.fromJson(item);
          }).toList(),
      perPage: settings['per_page'] ?? 20,
      listMode: mapListView[settings['list_mode']] ?? ProductViewType.list,
