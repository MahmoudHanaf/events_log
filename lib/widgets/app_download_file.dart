...
class _AppDownloadFileState extends State<AppDownloadFile> {
  ...
  void _downloadFile() async {
    final response = await Api.requestDownloadFile(
      file: widget.file,
      progress: (percent) {
        setState(() {
          _percent = percent;
  ...
  Widget _buildDownloading() {
    return SizedBox(
      width: 24,
      height: 24,
      child: CircularProgressIndicator(strokeWidth: 2, value: _percent),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(child: widget.child ?? _buildChild());
  }
}
