...
class BookingRepository {
  ...
  static Future<List?> loadList({
    ...
    final response = await Api.requestBookingList(params, request: request);
    if (response.success) {
      final attr = response.origin['attr'];
      final list =
          List.from(response.origin['data'] ?? []).map((item) {
      ...
      final listSort =
          List.from(attr['sort'] ?? []).map((item) {
            return SortModel.fromJson(item);
          }).toList();

      final listStatus =
          List.from(attr['status'] ?? []).map((item) {
            return SortModel.fromJson(item);
          }).toList();
