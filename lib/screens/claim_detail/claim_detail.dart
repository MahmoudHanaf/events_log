...
class ClaimDetail extends StatefulWidget {
  const ClaimDetail({super.key, required this.item});
...
class _ClaimDetailState extends State<ClaimDetail> {
  ...
  void _paymentClaim() async {
    ...
    if (payment != null) {
      ...
      await Navigator.pushNamed(context, Routes.payment, arguments: payment);
      _claimDetailCubit.onLoad(widget.item.id);
    }
  }
    ...
    return BlocBuilder<ClaimDetailCubit, ClaimDetailState>(
      ...
      builder: (context, state) {
        ...
        return Scaffold(
          appBar: AppBar(
            centerTitle: true,
            title: Text(Translate.of(context).translate('claim_detail')),
          ),
          body: content,
