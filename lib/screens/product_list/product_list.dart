...
class _ProductListState extends State<ProductList> {
    ...
    if (item.gps != null) {
      ...
      _mapController!.animateCamera(
        CameraUpdate.newCameraPosition(
          CameraPosition(
            target: LatLng(item.gps!.latitude, item.gps!.longitude),
            zoom: 15.0,
          ),
        ),
      );
  ...
  ///_build Item
  Widget _buildItem({ProductModel? item, required ProductViewType type}) {
    switch (type) {
      case ProductViewType.list:
        if (item != null) {
          return Container(
        ...
        return Container(
          padding: const EdgeInsets.symmetric(horizontal: 16),
          child: AppProductItem(type: _listMode),
        );
      default:
        if (item != null) {
        ...
        return AppProductItem(type: _listMode);
    }
  }
          ...
          if (state.list.isNotEmpty) {
            ...
            for (var item in state.list) {
              if (item.gps != null) {
                ...
                final marker = Marker(
                  markerId: markerId,
                  position: LatLng(item.gps!.latitude, item.gps!.longitude),
