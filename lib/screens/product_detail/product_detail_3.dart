...
class ProductDetail3 extends StatefulWidget {
  const ProductDetail3({super.key, required this.item});

  final ProductModel item;

  @override
  State<ProductDetail3> createState() {
    return _ProductDetailState();
  }
}

class _ProductDetailState extends State<ProductDetail3> {
  ...
  void _showMessage(String message) async {
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text(Translate.of(context).translate('explore_product')),
  ...
  void _onShare(ProductModel item) async {
    ...
    if (result == 'share') {
      Share.share('Check out my item ${item.link}', subject: 'PassionUI');
    } else if (result == 'copy') {
      _onCopy(item);
    }
  }
  ...
  void _onPhotoPreview(ProductModel item) {
    Navigator.pushNamed(context, Routes.gallery, arguments: item);
  }
  ...
  void _onBooking() async {
    ...
    if (!mounted) return;
    Navigator.pushNamed(context, Routes.booking, arguments: widget.item.id);
  }

  ///On navigate map
  void _onLocation(ProductModel item) {
    Navigator.pushNamed(context, Routes.gpsPicker, arguments: item.gps);
  }
                  ...
                  children: [
                    ...
                    Column(
                      children: [
                        AppListTitle(
                          title:
                              '${Translate.of(context).translate('call')} $phone',
                          leading: SizedBox(
                            height: 32,
                            width: 32,
        ...
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: ListBody(
              children: <Widget>[
                Text(
                  '${Translate.of(context).translate('install')} $appName ?',
  ...
  void _onClaim() async {
    ...
    if (!mounted) return;
    Navigator.pushNamed(context, Routes.claim, arguments: widget.item);
  }
  ...
  Widget _buildContent(ProductModel? product) {
    ...
    Widget banner = AppPlaceholder(child: Container(color: Colors.white));
    ...
    Widget favorite = const AppPlaceholder(
    ...
    if (product != null) {
      ...
      if (product.gps != null) {
        ...
        final markerId = MarkerId(product.title);
        final position = LatLng(
          product.gps?.latitude ?? 40.697403,
          product.gps?.longitude ?? -74.1201063,
        );
      ...
      if (product.status.isNotEmpty) {
        status = AppTag(product.status, type: TagType.status);
      }
              ...
              children:
                  product.tags.map((item) {
                    return IntrinsicWidth(
                      child: AppTag(item.title, type: TagType.chip),
                    );
                  }).toList(),
            ),
          ],
            ...
            Visibility(
              ...
              child: Padding(
                ...
                child: Wrap(
                  ...
                  runSpacing: 8,
                  children:
                      product.socials.entries.map((entry) {
                        return InkWell(
                          onTap: () {
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              shape: BoxShape.circle,
                              image: DecorationImage(
                                image: AssetImage(_exportSocial(entry.key)),
                              ),
                            ),
                          ),
                        );
        ...
        children: <Widget>[
          ...
          InkWell(
            ...
            child: Row(
              crossAxisAlignment: CrossAxisAlignment.center,
              children: <Widget>[
                AppTag("${product.rate}", type: TagType.rate),
                const SizedBox(width: 4),
                RatingBar.builder(
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: <Widget>[dateEstablish, priceRange],
                      ),
                      tags,
                    ],
                  ),
