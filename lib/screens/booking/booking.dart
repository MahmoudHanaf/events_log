...
class _BookingState extends State<Booking> {
  ...
  Future<bool> _onCalcPrice(FormSuccess form) async {
    final price = await _bookingCubit.calcPrice(id: widget.id, form: form);
    if (price != null) {
      setState(() {
  ...
  ///On next
  void _onNext({required FormSuccess form, required int step}) async {
    Utils.hiddenKeyboard(context);

    if (step == 0) {
      if (form.bookingStyle.adult == null) {
        AppBloc.messageBloc.add(MessageEvent(message: 'choose_adults_message'));
        return;
      }
      if (form.bookingStyle is StandardBookingModel) {
        final style = form.bookingStyle as StandardBookingModel;
        if (style.startDate == null) {
          AppBloc.messageBloc.add(MessageEvent(message: 'choose_date_message'));
          return;
        }
        if (style.startTime == null) {
          AppBloc.messageBloc.add(MessageEvent(message: 'choose_time_message'));
          return;
        }
      ...
      child: Column(
        children: [
          ...
          AppTextInput(
            ...
            onSubmitted: (text) {
              Utils.fieldFocusChange(context, _focusFirstName, _focusLastName);
            },
          ),
          const SizedBox(height: 12),
          AppTextInput(
            ...
            onSubmitted: (text) {
              Utils.fieldFocusChange(context, _focusLastName, _focusPhone);
            },
          ),
          const SizedBox(height: 12),
          AppTextInput(
            ...
            onSubmitted: (text) {
              Utils.fieldFocusChange(context, _focusPhone, _focusEmail);
            },
          ),
          const SizedBox(height: 12),
          AppTextInput(
            ...
            onSubmitted: (text) {
              Utils.fieldFocusChange(context, _focusEmail, _focusAddress);
            },
          ),
          const SizedBox(height: 12),
          AppTextInput(
            ...
            textInputAction: TextInputAction.next,
            onSubmitted: (text) {
              Utils.fieldFocusChange(context, _focusAddress, _focusMessage);
            },
          ),
          const SizedBox(height: 12),
          AppTextInput(
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            Translate.of(context).translate('booking_success_message'),
        ...
        if (form is FormSuccess) {
          ...
          content = Column(
            children: [
              ...
              AppStepper(active: _active, list: step),
              const SizedBox(height: 8),
              Expanded(
                child: SingleChildScrollView(child: _buildContent(form)),
              ),
            ],
          );
        }
