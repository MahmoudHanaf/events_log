...
class OpenTime extends StatefulWidget {
  ...
  const OpenTime({super.key, this.selected});

  @override
  State<OpenTime> createState() {
...
class _OpenTimeState extends State<OpenTime> {
    ...
    if (widget.selected != null) {
      ...
      _time = [
        OpenTimeModel(
          ...
          key: 'mon',
          schedule: [
            ScheduleModel(start: _defaultStartTime, end: _defaultEndTime),
          ],
        ),
        OpenTimeModel(
          dayOfWeek: 2,
  ...
  void _onTimePicker(TimeOfDay time, Function(TimeOfDay) callback) async {
    final picked = await showTimePicker(initialTime: time, context: context);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text(Translate.of(context).translate('open_time')),
        actions: [
          AppButton(
